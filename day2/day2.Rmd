...
### Individual measurement for each group
...
.p2. == .p13. -> Constraint with the largest $\chi^2$ reduction

What do the parameter labels (.p2. & .p13.) mean?
...
Obviously, we achieve the largest $\chi^2$ reduction in the metric invariance model by freely calculating the factor loading of item ipudrst, because there is a clear difference between group CZ and DE.
...
universalism_fit_metric_partial <- cfa(
    model = "
    group: [CZ]
    uni =~ NA*ipeqopt + a*ipeqopt + NA*ipudrst + c*impenv
    uni ~~ 1*uni
    group: [DE]
    uni =~ NA*ipeqopt + a*ipeqopt + NA*ipudrst + c*impenv
    uni ~~ 1*uni
...
```
CFI and RMSEA differences are too large according to @chen_sensitivity_2007, but overall acceptable. As a result, it is due to our consideration whether we have a metric measurement invariance here. It can be argued that the deviation in the CFI and RMSEA is negligible and the measurement model with metric invariance has a very good data fit. In this case we follow this reasoning and check further for scalar measurement invariance.
...
This also shows that we have to weigh up a rather critical case. The CFI and RMSEA deteriorate significantly and the fit to the data is also borderline. In this respect, it is a question of the researcher which point of view he wants to represent. As an example, we assume scalar measurement invariance here and consider the relative latent means, although we would rather assume no partial scalar measurement invariance here. In this case we would rather choose to use an approximate method.
